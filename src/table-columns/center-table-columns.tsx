...
export const columns = (onEdit: (center: Center) => void): ColumnDef<Center>[] => [
  {
    accessorKey: "id",
    header: "ID",
    cell: ({ row }) => <span className="text-sm text-zinc-800">{row.getValue("id")}</span>,
  },
  {
    accessorKey: "centerName",
    header: "School Name",
    cell: ({ row }) => (
      <span className="text-sm font-medium text-zinc-900">{row.getValue("centerName")}</span>
    ),
  },
  {
    accessorKey: "block",
    header: "Block",
    cell: ({ row }) => <span className="text-sm text-zinc-700">{row.getValue("block")}</span>,
  },
  {
    accessorKey: "subDivision",
    header: "Sub Division",
    cell: ({ row }) => <span className="text-sm text-zinc-700">{row.getValue("subDivision")}</span>,
  },
  {
    id: "actions",
    header: "Action",
          ...
          <PopoverContent align="end" className="w-48 p-2 bg-white border border-zinc-200 shadow-lg rounded-md">
            <div className="flex flex-col space-y-1">
              ...
              <Button
                variant="ghost"
                className="justify-start text-left text-sm hover:bg-zinc-100"
                onClick={() => alert(`Adding Student to ${center.centerName}`)}
              >
